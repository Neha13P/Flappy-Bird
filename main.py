...
def welcomeScreen():
    ...
    while True:
        for event in pygame.event.get():
            # Click on cross button closes the game
            if event.type == QUIT or (event.type == KEYDOWN and event.key == K_ESCAPE):
                pygame.quit()
                sys.exit()

            # Starts the game if the user clicks on up key or space
...
def mainGame():
    ...
    playerFlapped = False  # Is true only when the bird flaps
    ...
    while True:
        ...
        crashTest = isCollide(playerx, playery, upperPipes,lowerPipes)  # This function will return true if the player crashes
        if crashTest:
            return
...
if __name__ == "__main__":
    # Main point from where the game starts
    ...
    while True:
        welcomeScreen()  # Shows welcome screen to the user until he clicks a button
